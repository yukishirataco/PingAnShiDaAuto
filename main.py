...
headers = {
    ...
    'XMLHttpRequest',
    'Origin':
    'http://stu.gac.upc.edu.cn:8089',
    'Connection':
    'keep-alive',
    'Referer':
    'http://stu.gac.upc.edu.cn:8089/xswc&appid=200200819124942787&state=2',
}
...
def getStudentInfo(uid, pwd):
    ...
    usrdata = {'username': uid, 'password': pwd}
    response = session.post('https://app.upc.edu.cn/uc/wap/login/check',
                            headers=headers,
                            data=usrdata)
    info = session.get(
        "https://app.upc.edu.cn/uc/api/oauth/index?redirect=http://stu.gac.upc.edu.cn:8089/xswc&appid=200200819124942787&state=2"
    )
    code = re.findall(r"^\s+var\scode\s=\s.{33}.;$", info.text,
                      re.M)[0].strip().split('"')[1]  # 不用必死4，看我怎么用正则扒人参数
    # 妈的，这个code是一次性的
    xs = requests.post("http://stu.gac.upc.edu.cn:8089/stuqj/getXsMess",
                       headers=headers,
                       data=dict(code=code)).json()
    xs['data']['code'] = code
    return xs['data']
...
def AbsentReq(days):
    ...
    print("你将申请在平安系统上申请从{}到{}的进出请假，共{}天。".format(dates[0][:10], dates[-1][:10],
                                                 days))
    now = None
    after = None
    for i in dates:
        ...
        response = requests.post(
            'http://stu.gac.upc.edu.cn:8089/stuqj/addQjMess',
            headers=headers,
            data=data)
        now = datetime.datetime.today()
        if response.json()['resultStat'] == "success":
            print('[信息][{}] 用时 {:.2} s,请假时间为 {} 的请假已成功，返回为{}.'.format(
                str(now.strftime("%Y-%m-%d %H:%I:%S")),
                time.time() - now, data['stuStartTime'], str(response.json())))
        else:
            print('[信息][{}] 用时 {:.2} s,请假失败，返回为: {}'.format(
                str(now.strftime("%Y-%m-%d %H:%I:%S")),
                time.time() - nowtime,
...
if __name__ == '__main__':
    ...
    try:
        ...
        jwxt = requests.get("http://jwxt.upc.edu.cn",
                            timeout=10)  #直接访问一个外网没法访问的校内资源试试看
    except ConnectTimeout as e:
        print("访问校内资源出现超时！".format(e))
    ...
    print("不要大力拍打或者滑动哦")
    print('')
    # 你可以直接修改下面的两个账号密码为你自己的数字石大账号密码，你也可以删掉下面的两个字符串值然后自己在使用时填写。
    stu_id = ''
    stu_pwd = ''
